...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>XS hub เราเป็นค่ายฟรีเจ้าใหม่<Color=/>"):Display()
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>กรุณาใช้ ตัวรันโหดๆ<Color=/>"):Display()
    wait(.24)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>มีDiscord แจ้งเตือนเร็วๆนี้<Color=/>"):Display()
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>ขอให้ใชขอให้ใช้สคริปต์อย่างมีควอย่างมีความสุขขอบคุณครับ<Color=/>"):Display()
    wait(.36)
end)

local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "N hub", HidePremium = false, IntroText = "N hub", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("ผู้แครกsrcนี้")
W:AddLabel("Xs hub SCRIPT")
W:AddLabel("Discord Xs hub")
local Section = W:AddSection({
    Name = "Status"
})
